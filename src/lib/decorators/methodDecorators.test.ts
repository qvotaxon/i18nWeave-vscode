...
suite('TraceMethod Decorator', () => {
    ...
    test('should start and finish a Sentry span', async () => {
        ...
        assert.strictEqual(startSpan.calledOnce, true, 'Sentry.startSpan should be called once');
        assert.deepStrictEqual(startSpan.firstCall.args[0], { op: 'myMethod', name: 'Executing myMethod' }, 'Sentry.startSpan should be called with correct arguments');
    });
    ...
    test('should capture exceptions with Sentry', async () => {
        ...
        try {
            ...
            assert.strictEqual(captureException.calledOnce, true, 'Sentry.captureException should be called once');
            assert.deepStrictEqual(captureException.firstCall.args[0].message, 'Test Error', 'Sentry.captureException should be called with the correct error');
        }
    });
});
